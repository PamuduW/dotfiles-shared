...
- Raise `CONTRACT` here whenever a consumer starts loading something this tree
  gained, and raise that consumer's required revision in the same batch. A
  consumer checks for *at least* its revision, so one ahead of it is fine and
  one behind is the failure: without the raise the consumer dies on a bare
  `No such file` instead of the version mismatch CONTRACT exists to report.
